...
import type { SurveyDetail, SurveyPhoto } from '@/lib/api/surveys.api';
import { Button } from '@/components/ui/button';
import StatusBadge from '@/components/common/StatusBadge';
import PageLoader from '@/components/common/PageLoader';
import { toast } from 'sonner';
import { ArrowLeft, ChevronRight, Image as ImageIcon, Camera, Clock, User } from 'lucide-react';
...
export default function SurveyDetailPage() {
  ...
  const fetchSurvey = useCallback(async () => {
    setLoading(true);
    try {
      const res = await surveysApi.getById(id as string);
      setSurvey(res.data);
      if (res.data.scenes.length > 0) {
        setSelectedScene(res.data.scenes[0].id);
      }
    } catch {
